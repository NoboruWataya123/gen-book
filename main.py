...
def generate_text(prompt, model="claude-3-sonnet-20240229", max_tokens=2000, temperature=0.7):
    headers = {
...
def create_epub(title, author, chapters, cover_image_path='cover.png'):
    ...
    # Save the EPUB file
    epub_file_path = os.path.join(os.path.dirname(__file__), f'{title}.epub')
...
def generate_book(writing_style, book_description, num_chapters):
    ...
    for i in range(num_chapters):
        ...
        time.sleep(2)  # Add a short delay to avoid hitting rate limits
